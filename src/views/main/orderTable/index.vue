  ...
  <div class="layout-container">
    ...
    <div class="layout-container-table">
      ...
      >
        ...
        <el-table-column prop="下单时间" label="下单时间" align="center" />
        <el-table-column prop="更新时间" label="更新时间" align="center" />
        <el-table-column prop="付款方式" label="付款方式" align="center" />
        <el-table-column prop="状态" label="状态" align="center">
          <template #default="scope">
            <el-tag type="success" v-show="scope.row['订单状态'] == '已发货'">已发货</el-tag>
            <el-tag type="primary" v-show="scope.row['订单状态'] == '已打包'">已打包</el-tag>
            <el-tag type="warning" v-show="scope.row['订单状态'] == '失联'">失联</el-tag>
            <el-tag type="danger" v-show="scope.row['订单状态'] == '无效'">无效</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="总价（NTS）" label="总价（NTS）" align="center" />
...
import {
  ...
  del_Logistics_Key,
  getData_Orders,
  getData_Orders_Key, del_Orders_Key
} from '@/api/table'
...
export default defineComponent({
  ...
  setup() {
    ...
    const handleExportExcel = () => {
      ...
      let header = ["地区", "物流", "订单号", "内部名", "总价（NTS）", "属性", "SKU", "购买产品数量", "留言备注", "下单时间", "订单状态", "邮寄方式", "运单号", "国家", "城市", "区域", "全地址", "邮编", "产品外文名称", "客户姓名", "主电话", "邮箱", "付款方式", "更新时间"];
      let data = table.map((item) => {
        let { 地区, 物流, 订单号, 内部名, 总价NTS, 属性, SKU, 购买产品数量, 留言备注, 下单时间, 订单状态, 邮寄方式, 运单号, 国家, 城市, 区域, 全地址, 邮编, 产品外文名称, 客户姓名, 主电话, 邮箱, 付款方式, 更新时间 } = item;
        return [地区, 物流, 订单号, 内部名, 总价NTS, 属性, SKU, 购买产品数量, 留言备注, 下单时间, 订单状态, 邮寄方式, 运单号, 国家, 城市, 区域, 全地址, 邮编, 产品外文名称, 客户姓名, 主电话, 邮箱, 付款方式, 更新时间];
      })
      aoaToSheetXlsx({
        data,
        header,
